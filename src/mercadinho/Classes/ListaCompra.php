...
class ListaCompra
{
    ...
    function adicionarNaLista($codigo, $quantidade, $conn){
        if ($quantidade > 0){
            ...
            if ($result and $result->num_rows > 0) { // produto válido
                ...
                echo "<h2>Código de produto inválido!</h2>";
            }
        }
        else {
 ...
 //-------------------------------------------------------------------------------------------------------------
    function devolverProduto($id_produto, $id_venda, $quantidade_trocar, $conn){
        ...
        $result = $conn->query($sql);
        if ($result and $result->num_rows <= 0) { echo "<h2>Erro: Venda não existe! </h2>"; }
        else if (!$result){ echo "<h2>Erro procurando venda </<h2>h2>"; }
        else {
            ...
            if ($result and $result->num_rows > 0) {
                ...
                if ($quantidade_vendida < $quantidade_trocar) {
                    echo ("<h2>Quantidade insuficiente para devolver! A quantidade vendida foi " . $row["quantidade"] . " </h2>");
                }
                        ...
                        if ($result) {
                            ...
                            if ($result) {
                                ...
                                if ($result) {
                                    ...
                                    echo ("<h1>O valor a ser devolvido é R$".$valor_devolvido."</h1>");
                                    ...
                                    $sql = "UPDATE vendas
                                            ...
                                            WHERE id_venda = '$id_venda'";
                                    $result = $conn->query($sql);
                                    if (!$result) {echo "<h2>Erro atualizando o valor total da compra</h2>";}
                                }
                                else { echo "<h2>Erro recuperando valor da venda</h2>"; }
                            }
                            else { echo "<h2>Erro inserindo produto na tabela devolucoes</h2>"; }
                        }
                        else { echo "<h2>Erro atualizando produtos devolucao </h2>"; }
                    }
                    else { echo"<h2>Erro atualizando produtos devolucao </h2>"; }
                }
            }
            else {
                echo ("<h2>Erro - Produto não consta na venda!</h2>");
            }
        }
    }
//--------------------------------------------------------------------------------------------------------------
        ...
        if ($result) {
            if ($result->num_rows > 0) {
                ...
                if ($result and $result->num_rows > 0) {
                    ...
                    $result = $conn->query($sql);
                    if ($result and $result->num_rows > 0) { // produto entregue ao cliente existe
                        $row = $result->fetch_assoc();
                        $quantidade_vendida = $row["quantidade"];
                                        ...
                                        if ($result) {
                                            ...
                                            if ($result) {
                                                ...
                                                if ($result) {
                                                    ...
                                                    if ($devolver > $receber) {
                                                        echo ("<h1>O valor a ser devolvido é R$".$devolver-$receber."</h1>");
                                                    }
                                                    else if ($devolver < $receber) {
                                                        echo ("<h1>O valor a ser recebido é R$".$receber-$devolver."</h1>");
                                                    }
                                                    else {
                                                        echo ("<h2>Nenhum valor deve ser recebido ou devolvido!"."</h2>");
                                                    }
                                                }
                                                else {echo "<h2>Erro ao trocar produto: " . $sql . "<br>" . $conn->error."</h2>";};
                                            }
                                            else {echo "<h2>Erro ao trocar produto: " . $sql . "<br>" . $conn->error."</h2>";};
                                        }
                                        else {echo "<h2>Erro ao trocar produto: " . $sql . "<br>" . $conn->error."</h2>";};
                    ...
                    echo "<h2> O novo produto não existe </h2>";
                }
            }
            else {
                echo "<h2> A venda procurada não existe </h2>";
            }
        }
        else {
            echo "Erro ao trocar produto: " . $sql . "<br>" . $conn->error;
        }
    }
...
//-------------------------------------------------------------------------------------------
    ...
    function finalizarCompra($conn){
        ...
        if ($result and $result->num_rows > 0) {
            ...
            echo '<h1>Nenhum produto na lista de compras!</h1>';
        }
    }
    ...
    function get_valor_total($conn){
        $sql = "SELECT valor, lista_compras.quantidade
                FROM lista_compras
                LEFT JOIN produtos
                ON lista_compras.id_produto = produtos.id_produto";
        $result = $conn->query($sql);
